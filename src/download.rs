...
pub fn verify_checksum(
    ...
    if actual_shasum != expected_shasum {
        return Err(format!("Checksum mismatch: expected {}, got {}", expected_shasum, actual_shasum).into());
    }
...
pub fn extract_tarball(
    ...
    let status = Command::new("tar")
        .args(["-xJf", archive_path.to_str().unwrap(), "-C", dest_dir.to_str().unwrap(), "--strip-components=1"])
        .status()?;
...
pub fn extract_zip(
    ...
    let status = Command::new("unzip")
        .args(["-o", archive_path.to_str().unwrap(), "-d", dest_dir.to_str().unwrap()])
        .status()?;
